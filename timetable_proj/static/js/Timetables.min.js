      ...
      timetable.forEach(function (values, index) {
        ...
        values.forEach(function (item, i) {
          ...
          if (merge && listMerge[i][index].length > 1) {
            ...
            // Updated by Tim 20230312
            ctext = listMerge[i][index].name;
            ctextarr = ctext.split("\n");
            var cname;
            for (m = 0; m < ctextarr.length; m++) {
              var coursenameElm = document.createElement("div");
              if (m == 0) {
                coursenameElm.style.fontSize = "16px";
                coursenameElm.style.fontWeight = "bold";
                cname = ctextarr[m];
              }
              coursenameElm.innerText = ctextarr[m];
              mergeDom.appendChild(coursenameElm);
            }
            for (k = 0; k < classArrHistory.length; k++) {
              if (cname == classArrHistory[k]) {
                mergeDom.style.backgroundColor = classPaletteHistory[k];
                break;
              } else {
                if (k == classArrHistory.length - 1) {
                  if (palette) {
                    mergeDom.style.backgroundColor = palette[paletteIndex];
                    classPaletteHistory.push(palette[paletteIndex]);
                    classArrHistory.push(cname);
                    paletteIndex++;
