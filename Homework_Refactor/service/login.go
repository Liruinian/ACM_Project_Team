...
func Login(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(200, gin.H{"code": 2001, "msg": "输入格式不正确"})
		return
	}
	if lForm.Usr == "" || lForm.Pass == "" {
		c.JSON(200, gin.H{"code": 2002, "msg": "用户名或密码不能为空"})
		return
	}
	var res []*tools.Login
	...
	if tools.ComparePwd(lDBForm.Password, lForm.Pass) {
		loginToken := tools.CreateToken("user_token", lDBForm.Username)
...
func Signup(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(200, gin.H{"code": 2011, "msg": "输入格式不正确"})
		return
	}
	sForm.Usertype = "user"

	if sForm.Username == "" || sForm.Email == "" || sForm.Phone == "" || sForm.Password == "" {
		c.JSON(200, gin.H{"code": 2012, "msg": "注册部分不能有空值"})
		return
	}
...
func GetUserInfo(c *gin.Context) {
	...
	if tools.VerifyUser(username, lToken) {
		var lList tools.Login
		DB.Table("login").Select("*").Where("username = ?", username).Scan(&lList)
		c.JSON(200, gin.H{"code": 2090, "msg": "success", "username": username, "usertype": lList.Usertype})
	} else {
